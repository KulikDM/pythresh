...
class GAMGMM(BaseThresholder):
    ...
    def __init__(self, n_contaminations=1000, n_draws=50, p0=0.01, phigh=0.01, high_gamma=0.15, gamma_lim=0.5, K=100, skip=False, steps=100, random_state=1234, verbose=False):
    ...
    def eval(self, decision):
        ...
        if (np.asarray(decision).ndim == 2) & (np.atleast_2d(decision).shape[1] > 1):
            ...
            score_space = np.vstack([score_space, np.zeros_like(score_space)]).T
    ...
    def _compute_gamma_posterior(self, decision):
        ...
        self.K = decision.shape[0] - 1 if np.shape(decision)[0] < self.K else self.K
        ...
        while True:
            ...
            idx_sortcomponents, meanstd = self._order_components(means, mean_precs, covariances, dgf)
            ...
            res = least_squares(self._find_delta_tau, x0=(-2, 1), args=(meanstd, alphas), bounds=((-50, -1), (0, 50)))

            delta, tau = res.x

            # Check that delta and tau are properly found, allowing for a 10% error
            p0Est, phighEst = self._check_delta_tau(delta, tau, meanstd, alphas)

            if ((p0Est < self.p0 * 1.1) & (p0Est > self.p0 * 0.9) & (phighEst < self.phigh * 1.1) & (phighEst > self.phigh * 0.9)) or (self.skip):
        ...
        gamma = self._sample_withexactprobs(means, mean_precs, covariances, dgf, delta, tau, w)
    ...
    def _order_components(self, means, mean_precs, covariances, dgf):
        ...
        for k in range(K):
            sample_mean_component = multivariate_normal.rvs(mean=means[k, :], cov=mean_std[k] ** 2, size=1000, random_state=self.random_state)
            sample_covariance = wishart.rvs(df=dgf[k], scale=covariances[k] / dgf[k], size=1000, random_state=self.random_state)

            var = np.array([np.diag(sample_covariance[i]) for i in range(1000)])

            meanstd[k] = np.mean([np.mean(sample_mean_component[:, m].reshape(-1) / (1 + np.sqrt(var[:, m].reshape(-1)))) for m in range(M)])
    ...
    def _sample_withexactprobs(self, means, mean_precs, covariances, dgf, delta, tau, w):
        ...
        while len(samples) < self.n_contaminations * (1 - self.p0):
            ...
            for k in range(K):
                ...
                sample_mean_component = multivariate_normal.rvs(mean=means[k, :], cov=mean_std[k] ** 2, size=1, random_state=10 * random_start + rnd)

                sample_covariance = wishart.rvs(df=dgf[k], scale=covariances[k] / dgf[k], size=1, random_state=10 * random_start + rnd)
        ...
        samples = np.concatenate((samples, np.zeros(self.n_contaminations - len(samples), np.float32)))
