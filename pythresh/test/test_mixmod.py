...
class TestMIXMOD(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.methods = ['mean', 'ks']
        cls.tols = [1e-3, 1e-5, 1e-8, 1e-12]
        cls.max_iters = [50, 100, 250, 500]

        cls.params = list(product(cls.all_scores, cls.methods,
                                  cls.tols, cls.max_iters))
    ...
    def test_eval(self):
        for scores, method, tol, max_iter in self.params:
            thres = MIXMOD(method=method, tol=tol, max_iter=max_iter)
