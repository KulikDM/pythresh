...
class TestOCSVM(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.models = ['poly', 'sgd']
        cls.degree = ['auto', 2, 3, 4, 7, 10]
        cls.gamma = ['auto', 0.1, 0.5, 0.9]
        cls.criterion = ['aic', 'bic']
        cls.nu = [0.1, 0.5, 0.9]
        cls.tol = [1e-1, 1e-3, 1e-5]

        cls.params = list(product(cls.all_scores, cls.models, cls.degree,
                                  cls.gamma, cls.criterion, cls.nu, cls.tol))
    ...
    def check_fitted_attributes(self, thres):
        ...
        self.assertIsNone(thres.thresh_)

    def test_eval(self):
        for scores, model, degree, gamma, criterion, nu, tol in self.params:
            thres = OCSVM(model=model, degree=degree,
