...
param_grid = list(product(factors, score_cases))
...
@pytest.mark.parametrize("factor,score_case", param_grid)
def test_eval(scores, factor, score_case):
    _, idx = score_case
    s = scores[idx]

    thres = ZSCORE(factor=factor)
    labels = thres.eval(s)
